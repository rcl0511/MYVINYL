...
import { getLPById, MOCK_LPS } from "@/lib/mock-data";
...
export default function PlayerPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      ytPlayerRef.current = new (window as any).YT.Player("yt-audio-player", {
        ...
        events: {
          ...
          onStateChange: (e: { data: number }) => {
            ...
            if (e.data === 0) {
              setTrackIdx((i) => (i + 1) % sideTracks.length);
              setProgress(0);
              setElapsed(0);
            }
          },
  ...
  useEffect(() => {
    function calc() {
      ...
      const base = Math.min(vw * 0.56, vh * 0.62, 640);
      setTurntableSize(Math.max(320, Math.round(base)));
    }
    calc();
  ...
  }, [state, currentTrack, totalSeconds, sideTracks.length]);
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      switch (e.code) {
        ...
        case "ArrowRight": setTrackIdx((i) => (i + 1) % sideTracks.length); break;
        case "ArrowLeft": setTrackIdx((i) => (i - 1 + sideTracks.length) % sideTracks.length); break;
  ...
  }, [sideTracks.length, resetHideTimer]);
  ...
  const lpIndex = MOCK_LPS.findIndex((l) => l.id === id);
  ...
  return (
    ...
    >
      ...
      {/* ── Top bar ── */}
      <div
        className="relative z-10 flex items-center justify-between px-6 pt-5 pb-2 transition-all duration-700"
      ...
      >
        ...
        <div className="text-center">
          <p className="text-white/80 font-semibold text-sm leading-tight">{lp.title}</p>
          <p className="text-white/40 text-xs mt-0.5">{lp.artist} · {lp.year}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs text-white/40 hover:text-white transition-colors"
            style={{ border: "1px solid rgba(255,255,255,0.1)" }}
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
      ...
      {/* ── Bottom controls ── */}
      <div
        className="relative z-10 px-6 pb-6 transition-all duration-700"
        style={{ opacity: uiVisible ? 1 : 0, pointerEvents: uiVisible ? "auto" : "none" }}
      >
        <PlayerControls
          state={state}
          ...
          onPrev={() => setTrackIdx((i) => (i - 1 + sideTracks.length) % sideTracks.length)}
          onNext={() => { setTrackIdx((i) => (i + 1) % sideTracks.length); setState("playing"); }}
      ...
      {/* ── Keyboard hint ── */}
      <div
        className="absolute bottom-3 right-4 flex gap-3 text-white/15 text-[10px] transition-all duration-700"
